...
def create_model_instance_SL_two_splits(dataset_type, model_type, worker_num, class_num=10):
    ...
    if dataset_type == 'CIFAR10':
        server1 = AlexNet_U_Shape(1, 11, class_num)
        server2 = AlexNet_U_Shape(11, -1, class_num)
        for net_i in range(worker_num):
            net = (AlexNet_U_Shape(0, 1, class_num))
            client_nets[net_i] = net
        return client_nets, server1, server2

    elif dataset_type == 'image100':
        server1 = VGG16_U_Shape(3, 6)
        server2 =  VGG16_U_Shape(6, -1)
        for net_i in range(worker_num):
            net = (VGG16_U_Shape(0, 3))
            client_nets[net_i] = net
        return client_nets, server1, server2
        ...
        server1 = CNN_HAR_U_Shape(1, 2)
        server2 = CNN_HAR_U_Shape(2, -1)
        for net_i in range(worker_num):
            net = (CNN_HAR_U_Shape(0, 1))
            client_nets[net_i] = net
        return client_nets, server1, server2
        ...
        server1 = M5_U_Shape(1, 2)
        server2 = M5_U_Shape(2, -1)
        for net_i in range(worker_num):
            net = (M5_U_Shape(0, 1))
            client_nets[net_i] = net
        return client_nets, server1, server2
